...
# Load the CSV file
input_file = r'C:\Users\lule896g\Desktop\Lucas Files\Time_Disp.CSV'
df = pd.read_csv(input_file, header=None)

input_z = r'C:\Abaqus_temp\scripts\coorZ_matrix.CSV'
...
colors = cmap(np.linspace(0, 1, 19))  # Generate 19 colors from the colormap
...
for delta in range(19):
    ...
    plt.title(f'Displacement vs Z Coordinate at Time t = {reshaped_df.iloc[delta, 0]}')
    #plt.legend()
...
frame_files = [os.path.join(frames_dir, f'frame_{delta:03d}.png') for delta in range(19)]
...
video = cv2.VideoWriter(video_filename, cv2.VideoWriter_fourcc(*'mp4v'), 2, (width, height))  # 2 FPS
